...
- **模板识别与排版体验优化**：
  - 修复绝对定位空间精雕模板（如 `简约单页18` 等 18 款模板）因为存在 `.yaml` 配置文件，在 `check-template-config` 中被误判定为普通的流式 `'yaml'` 引擎，导致前端 `isSpatial` 误判为 false，进而屏蔽了 `InteractiveCanvas` 可视化画布的严重 Bug。
  - 优化模板列表中的视觉标牌展示，将原先含混不清的“✨ 精雕”标牌，细分为 `🎨 自由精雕`（支持绝对定位交互画布）和 `📄 智能分页`（支持流式自动分页），彻底清除了用户的认知误区。
  - 将“隐私打码”和“历史快照”从 `isSpatial` 特有栏移至顶部的“通用主工具栏”，使得普通流式分页模板也能享受数据掩码打码预览、安全导出和多版本快照回滚服务。
