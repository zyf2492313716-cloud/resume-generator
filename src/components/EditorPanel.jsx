...
export default function EditorPanel({
  ...
  const handleAiExtract = async () => {
    setIsParsing(true);
    onNotification({ type: 'info', message: '正在启动智能解析引擎...' });

    try {
      const parsed = await parseResumeText(aiInput, aiConfig);
      setResumeData(parsed);
      onNotification({ type: 'success', message: '简历解析成功，已自动填入表单' });
      setActiveTab('form');
    } catch (err) {
      onNotification({
        type: 'warning',
        message: 'API 解析受限，已启用本地备用解析器'
      });
    } finally {
      setIsParsing(false);
    }
  ...
  const addArrayItem = (type, defaultObj) => {
    ...
    onNotification({ type: 'info', message: '已添加新条目' });
  };
  ...
  const removeArrayItem = (type, index) => {
    ...
    onNotification({ type: 'info', message: '已删除条目' });
  };
        ...
        {activeTab === 'ai' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', lineHeight: '1.5' }}>
              粘贴 AI 生成的简历文稿，点击一键提取，系统将自动解析并填充至预览画布。
            </div>
              ...
              {showApiSettings && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <label style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>API 端点</label>
                    <input
                      type="text"
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <label style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>API Key</label>
                    <input
                      type="password"
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <label style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>模型名称</label>
                    <input
                      type="text"
        ...
        {activeTab === 'form' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '14px', fontWeight: 700, borderBottom: '1px solid var(--border-glass)', paddingBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Briefcase size={15} style={{ color: 'var(--color-accent)' }} /> 工作经历
                </div>
                <button
              ...
              {resumeData.experience.map((exp, idx) => (
                <div key={idx} style={cardStyle}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    <input type="text" placeholder="公司" value={exp.company || ''} onChange={(e) => updateArrayItem('experience', idx, 'company', e.target.value)} style={smallInputStyle} />
                  ...
                  <input type="text" placeholder="职位" value={exp.role || ''} onChange={(e) => updateArrayItem('experience', idx, 'role', e.target.value)} style={{ ...smallInputStyle, marginTop: '8px' }} />
                  <textarea placeholder="职责描述（- 开头自动列表）" value={exp.description || ''} onChange={(e) => updateArrayItem('experience', idx, 'description', e.target.value)} style={{ ...smallInputStyle, height: '70px', marginTop: '8px', resize: 'vertical' }} />
                </div>
              ))}
            </div>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '14px', fontWeight: 700, borderBottom: '1px solid var(--border-glass)', paddingBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <FolderGit size={15} style={{ color: 'var(--color-accent)' }} /> 项目经验
                </div>
                <button
                  onClick={() => addArrayItem('projects', { name: '新项目', role: '开发负责人', date: '2024.01 - 2024.05', description: '项目详情...' })}
                  style={addBtnStyle}
                >
                  <Plus size={12} /> 添加
                </button>
              ...
              {resumeData.projects.map((proj, idx) => (
                <div key={idx} style={cardStyle}>
                  ...
                  <textarea placeholder="项目详情（- 开头自动列表）" value={proj.description || ''} onChange={(e) => updateArrayItem('projects', idx, 'description', e.target.value)} style={{ ...smallInputStyle, height: '70px', marginTop: '8px', resize: 'vertical' }} />
                </div>
              ))}
            </div>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '14px', fontWeight: 700, borderBottom: '1px solid var(--border-glass)', paddingBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <GraduationCap size={15} style={{ color: 'var(--color-accent)' }} /> 教育背景
                </div>
                <button
              ...
              {resumeData.education.map((edu, idx) => (
                <div key={idx} style={cardStyle}>
                  <div style={cardHeaderStyle}>
                    <span style={{ fontWeight: 600, fontSize: '12px' }}>教育 #{idx + 1}</span>
                    <div style={{ display: 'flex', gap: '4px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    <input type="text" placeholder="学校" value={edu.school || ''} onChange={(e) => updateArrayItem('education', idx, 'school', e.target.value)} style={smallInputStyle} />
                    <input type="text" placeholder="时间" value={edu.date || ''} onChange={(e) => updateArrayItem('education', idx, 'date', e.target.value)} style={smallInputStyle} />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '8px' }}>
                    <input type="text" placeholder="专业" value={edu.major || ''} onChange={(e) => updateArrayItem('education', idx, 'major', e.target.value)} style={smallInputStyle} />
                    <input type="text" placeholder="学历" value={edu.degree || ''} onChange={(e) => updateArrayItem('education', idx, 'degree', e.target.value)} style={smallInputStyle} />
                  </div>
                  <textarea placeholder="校内表现 (选填)" value={edu.description || ''} onChange={(e) => updateArrayItem('education', idx, 'description', e.target.value)} style={{ ...smallInputStyle, height: '50px', marginTop: '8px', resize: 'vertical' }} />
                </div>
              ))}
            </div>
